...
${"".padEnd(offset)}${"^".repeat(len)}`;
  ...
  var Tokenizer = class {
    ...
    stateInSpecialTag(c2) {
      if (this.sequenceIndex === this.currentSequence.length) {
        if (c2 === CharCodes3.Gt || isWhitespace(c2)) {
      ...
      if ((c2 | 32) === this.currentSequence[this.sequenceIndex]) {
        ...
        if (this.currentSequence === Sequences.TitleEnd) {
          if (this.decodeEntities && c2 === CharCodes3.Amp) {
            this.state = State.BeforeEntity;
          }
        ...
        this.sequenceIndex = Number(c2 === CharCodes3.Lt);
      }
    }
    ...
    stateBeforeTagName(c2) {
      if (c2 === CharCodes3.ExclamationMark) {
        this.state = State.BeforeDeclaration;
        this.sectionStart = this.index + 1;
      } else if (c2 === CharCodes3.Questionmark) {
    ...
    stateAfterClosingTagName(c2) {
      if (c2 === CharCodes3.Gt || this.fastForwardTo(CharCodes3.Gt)) {
        this.state = State.Text;
        this.baseState = State.Text;
        this.sectionStart = this.index + 1;
      }
    ...
    stateInAttributeName(c2) {
      if (c2 === CharCodes3.Eq || isEndOfTagSection(c2)) {
    ...
    stateAfterAttributeName(c2) {
      ...
      } else if (c2 === CharCodes3.Slash || c2 === CharCodes3.Gt) {
    ...
    stateBeforeAttributeValue(c2) {
      if (c2 === CharCodes3.DoubleQuote) {
        this.state = State.InAttributeValueDq;
        this.sectionStart = this.index + 1;
      } else if (c2 === CharCodes3.SingleQuote) {
    ...
    stateInAttributeValueDoubleQuotes(c2) {
      this.handleInAttributeValue(c2, CharCodes3.DoubleQuote);
    }
    stateInAttributeValueSingleQuotes(c2) {
      this.handleInAttributeValue(c2, CharCodes3.SingleQuote);
    }
    stateInAttributeValueNoQuotes(c2) {
      if (isWhitespace(c2) || c2 === CharCodes3.Gt) {
        this.cbs.onattribdata(this.sectionStart, this.index);
        this.sectionStart = -1;
    ...
    stateBeforeDeclaration(c2) {
      if (c2 === CharCodes3.OpeningSquareBracket) {
    ...
    stateBeforeEntity(c2) {
      this.entityExcess = 1;
      this.entityResult = 0;
      if (c2 === CharCodes3.Number) {
        this.state = State.BeforeNumericEntity;
      } else if (c2 === CharCodes3.Amp) {
      } else {
        this.trieIndex = 0;
    ...
    stateInNamedEntity(c2) {
      ...
      const masked = this.trieCurrent & BinTrieFlags2.VALUE_LENGTH;
      if (masked) {
        const valueLength = (masked >> 14) - 1;
        if (!this.allowLegacyEntity() && c2 !== CharCodes3.Semi) {
    ...
    emitNumericEntity(strict) {
      ...
      if (numberStart !== this.index) {
        ...
        this.emitCodePoint(replaceCodePoint2(this.entityResult));
      }
      this.state = this.baseState;
    }
    stateInNumericEntity(c2) {
      if (c2 === CharCodes3.Semi) {
        this.emitNumericEntity(true);
      } else if (isNumber3(c2)) {
        this.entityResult = this.entityResult * 10 + (c2 - CharCodes3.Zero);
        this.entityExcess++;
      } else {
  ...
  var EntityDecoder3 = class {
    ...
    write(str, offset) {
      switch (this.state) {
        case EntityDecoderState3.EntityStart: {
          if (str.charCodeAt(offset) === CharCodes4.NUM) {
    ...
    stateNumericStart(str, offset) {
      ...
      if ((str.charCodeAt(offset) | TO_LOWER_BIT3) === CharCodes4.LOWER_X) {
    ...
    for (const chapter of chapterData) {
      ...
      if (chapterMap.has(chapNum)) {
        if (chapterScore > chapterMap.get(chapNum).score) {
          chapterMap.set(chapNum, { score: chapterScore, chapter });
        }
      } else {
        chapterMap.set(chapNum, { score: chapterScore, chapter });
      }
  ...
  var ComicK = class {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      for (const s2 of sections) {
        ...
        parseHomeSections(data, s2.id, sectionCallback);
      }
    }
